...
- Arg 2 is a single-line standard Base64 string (no spaces); read it with `sys.argv[2]` (Python), `args[1]` (Java), or `process.argv[3]` (Node — `argv[1]` is the script path)
...
Requires **JDK 11+**; you can run the single source file directly (no `javac` step). `args[0]` is the mode, `args[1]` is the Base64 payload.
