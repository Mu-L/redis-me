...
const zhConfigTip = {
  "include": "引入一个或多个其他配置文件，支持通配符，包含的文件按字母顺序加载，未匹配到文件时无错误提示",
  ...
  "protected-mode": "安全保护模式，开启且默认用户无密码时，仅允许本地连接（127.0.0.1、::1 或 Unix 域套接字），默认启用",
  "enable-protected-configs": "控制敏感配置指令的可修改权限，可选值 no（默认，禁止所有连接修改）、yes（允许所有连接修改）、local（仅允许本地连接修改）",
  ...
  "tcp-keepalive": "TCP 保活时间（秒），非 0 值启用 SO_KEEPALIVE 选项，用于检测死连接和维持网络连接，默认 300",
  "socket-mark-id": "为监听套接字标记 ID，支持高级路由和过滤，Linux 对应连接标记、FreeBSD 对应套接字 cookie ID、OpenBSD 对应路由表 ID，默认 0",
  ...
  "tls-dh-params-file": "DH 参数文件路径，用于支持旧版 OpenSSL（<3.0）的 Diffie-Hellman 密钥交换，新版不推荐配置",
  "tls-ca-cert-file": "CA 证书文件路径，用于认证 TLS 客户端和节点，Redis 不默认使用系统 CA 配置",
  "tls-ca-cert-dir": "CA 证书目录路径，功能同 tls-ca-cert-file",
  "tls-auth-clients": "控制 TLS 客户端证书认证要求，可选值 no（不要求也不接受客户端证书）、optional（可选，提供则需有效），默认要求客户端证书",
  "tls-replication": "是否为复制链路启用 TLS，默认 no",
  "tls-cluster": "是否为集群总线启用 TLS，默认 no",
  "tls-protocols": "支持的 TLS 版本，可选 TLSv1、TLSv1.1、TLSv1.2、TLSv1.3（OpenSSL≥1.1.1），默认启用 TLSv1.2 和 TLSv1.3",
  ...
  "pidfile": "pid 文件路径，非守护进程模式下未指定则不生成，守护进程模式默认 /var/run/redis.pid，推荐现代 Linux 系统使用 /run/redis.pid",
  ...
  "proc-title-template": "进程标题模板，支持 {title}、{listen-addr}、{server-mode}、{port}、{tls-port}、{unixsocket}、{config-file} 变量，默认{title} {listen-addr} {server-mode}",
  ...
  "stop-writes-on-bgsave-error": "RDB 快照启用时，若后台保存失败是否停止接受写入，默认 yes，确保数据持久化正常",
  "rdbcompression": "是否使用 LZF 压缩 RDB 文件中的字符串对象，默认 yes，禁用可节省 CPU 但增大文件体积",
  "rdbchecksum": "是否在 RDB 文件末尾添加 CRC64 校验和，默认 yes，禁用可提升读写性能但降低文件完整性校验能力",
  "sanitize-dump-payload": "加载 RDB 或 RESTORE 数据时是否执行完整校验，可选 no（默认）、yes、clients，用于减少后续命令执行时的断言或崩溃风险",
  "dbfilename": "RDB 文件名称，默认 dump.rdb",
  "rdb-del-sync-files": "无持久化启用时，是否删除复制过程中生成的 RDB 文件，默认 no，仅在 AOF 和 RDB 均禁用时生效",
  "dir": "工作目录，RDB 文件、AOF 文件均存储在此目录，默认./",
  "replicaof": "配置当前实例为从节点，指定主节点的 IP 和端口",
  "masterauth": "主节点的密码，主节点启用密码时需配置",
  "masteruser": "复制使用的主节点用户名，Redis 6 及以上支持 ACL 时使用，用于执行 PSYNC 等复制所需命令",
  "replica-serve-stale-data": "从节点与主节点断开或复制中时是否返回过期数据，默认 yes，设为 no 则除部分命令外拒绝数据访问命令",
  "replica-read-only": "从节点是否只读，默认 yes，Redis 2.6 起默认只读",
  "repl-diskless-sync": "复制全量同步时使用的策略，yes 表示无盘同步（直接通过套接字传输 RDB），no 表示基于磁盘同步，默认 yes",
  "repl-diskless-sync-delay": "无盘同步时，等待更多从节点连接的延迟时间（秒），默认 5，设为 0 立即开始传输",
  "repl-diskless-sync-max-replicas": "无盘同步延迟期间，达到指定数量的从节点连接后提前开始同步，默认 0 表示不限制",
  "repl-diskless-load": "从节点加载主节点 RDB 数据的方式，可选 disabled（默认，先存储到磁盘再加载）、swapdb（内存中并行加载，需足够内存）、on-empty-db（仅空数据库时无盘加载）",
  "repl-ping-replica-period": "主节点向从节点发送 PING 的间隔时间（秒），默认 10",
  "repl-timeout": "复制超时时间（秒），涵盖 SYNC 批量传输、主从节点通信等场景，默认 60，需大于 repl-ping-replica-period",
  "repl-disable-tcp-nodelay": "全量同步后是否禁用从节点套接字的 TCP_NODELAY 选项，默认 no（低延迟优先），yes 可减少带宽占用但增加数据延迟",
  "repl-backlog-size": "复制积压缓冲区大小，用于从节点重连后的部分同步，默认 1mb，仅当有从节点连接时分配",
  "repl-backlog-ttl": "最后一个从节点断开后，复制积压缓冲区的释放时间（秒），默认 3600，0 表示永不释放",
  "replica-full-sync-buffer-limit": "全量同步时从节点可累积的复制数据流大小，0 表示继承主节点 client-output-buffer-limit replica 的硬限制，默认 0",
  "replica-priority": "从节点优先级，用于 Redis Sentinel 故障转移时选择主节点，值越小优先级越高，0 表示不可晋升为主节点，默认 100",
  "propagation-error-behavior": "处理复制流或 AOF 文件中命令执行错误的行为，可选 ignore（默认）、panic、panic-on-replicas",
  "replica-ignore-disk-write-errors": "从节点写入磁盘失败时是否忽略错误，默认 no（崩溃），yes 仅日志警告并执行命令",
  "replica-announced": "从节点是否被 Redis Sentinel 纳入报告，默认 yes，设为 no 则不被 sentinel replicas 命令显示，但仍可晋升为主节点（需配合 replica-priority 0 禁用晋升）",
  "min-replicas-to-write": "主节点接受写入所需的最小在线从节点数量，默认 0（禁用），需与 min-replicas-max-lag 配合使用",
  ...
  "tracking-table-max-keys": "客户端缓存键跟踪的无效表最大键数量，默认 1000000，0 表示无限制，广播模式下此配置无效",
  "acllog-max-len": "ACL 日志的最大条目数，用于记录 ACL 相关的失败命令和认证事件，默认 128",
  "aclfile": "外部 ACL 用户配置文件路径，不可与当前配置文件中的用户配置同时使用",
  "requirepass": "默认用户的密码，Redis 6 及以上为 ACL 系统的兼容层，与 aclfile 和 ACL LOAD 命令不兼容",
  "acl-pubsub-default": "新用户的默认 Pub/Sub 频道权限，可选 allchannels（允许所有频道）、resetchannels（禁止所有频道），Redis 7.0 默认 resetchannels",
  "rename-command": "重命名或禁用命令（已废弃），建议使用 ACL 控制命令权限，格式为 “rename-command 原命令 新命令”，新命令设为空字符串则禁用该命令",
  "maxclients": "最大同时连接客户端数，默认 10000，实际受系统文件描述符限制，Redis Cluster 模式下需预留集群总线连接资源",
  "maxmemory": "Redis 使用的最大内存限制，达到限制后按 maxmemory-policy 执行键淘汰，默认无限制",
  "maxmemory-policy": "内存达到 maxmemory 后的键淘汰策略，可选 volatile-lru、allkeys-lru、volatile-lfu、allkeys-lfu、volatile-random、allkeys-random、volatile-ttl、noeviction（默认）",
  "maxmemory-samples": "LRU/LFU/TTL 淘汰算法的采样数量，默认 5，值越大越接近真实结果但 CPU 消耗越高，最大 64",
  "maxmemory-eviction-tenacity": "键淘汰处理的激进程度，0 为最低延迟、10 为默认、100 为优先处理淘汰忽略延迟，默认 10",
  "replica-ignore-maxmemory": "从节点是否忽略自身 maxmemory 配置，默认 yes（由主节点处理淘汰），仅当从节点可写或需独立内存限制时修改",
  "active-expire-effort": "主动过期键清理的力度，默认 1，取值 1-10，值越大清理越彻底但 CPU 消耗越高",
  ...
  "lazyfree-lazy-user-flush": "用户执行 FLUSHDB/FLUSHALL/SCRIPT FLUSH/FUNCTION FLUSH 时未指定同步 / 异步标志，是否默认异步删除，默认 no",
  "io-threads": "I/O 线程数，用于处理客户端套接字的读写和协议解析，默认禁用（1 表示仅主线程），建议 4 核及以上服务器启用，预留 1 个空闲核心",
  "oom-score-adj": "是否控制内核 OOM killer 的进程优先级，可选 no（默认）、yes（等同于 relative）、absolute、relative，用于优先淘汰后台子进程和从节点",
  "oom-score-adj-values": "OOM 优先级调整值，依次为主节点、从节点、后台子进程，取值 - 2000 至 2000，默认 0 200 800",
  "disable-thp": "是否禁用 Redis 进程的透明大页（THP），默认 yes，避免 fork 和写时复制（CoW）导致的延迟问题",
  "appendonly": "是否启用 AOF 持久化，默认 no，AOF 与 RDB 可同时启用，启动时优先加载 AOF",
  "appendfilename": "AOF 文件的基础名称，Redis 7 及以上使用多文件格式（基础文件、增量文件、清单文件），默认\"appendonly.aof\"",
  "appenddirname": "AOF 文件存储目录，默认\"appendonlydir\"",
  "appendfsync": "AOF 文件的 fsync 策略，可选 always（每次写入同步，最安全）、everysec（每秒同步，默认）、no（由系统控制，最快）",
  "no-appendfsync-on-rewrite": "BGSAVE 或 BGREWRITEAOF 执行时，是否暂停 AOF 的 fsync，默认 no（优先数据安全），yes 可缓解 I/O 阻塞但降低耐久性",
  "auto-aof-rewrite-percentage": "AOF 自动重写的触发百分比，基于上次重写后的文件大小，默认 100（增长 100%），0 禁用自动重写",
  "auto-aof-rewrite-min-size": "AOF 自动重写的最小文件大小，默认 64mb",
  "aof-load-truncated": "启动时加载被截断的 AOF 文件是否继续，默认 yes（加载可用部分并日志提示），no 则直接报错",
  "aof-load-corrupt-tail-max-size": "启动时允许自动修复的 AOF 文件尾部损坏字节数，默认 0（禁用自动修复），超出则需手动处理",
  "aof-use-rdb-preamble": "AOF 基础文件是否使用 RDB 格式，默认 yes，RDB 格式更快更高效",
  "aof-timestamp-enabled": "是否在 AOF 中记录时间戳注释，支持按时间点恢复，默认 no，启用后可能与部分 AOF 解析器不兼容",
  "shutdown-timeout": "关闭时等待从节点同步的最大时间（秒），仅对有从节点的实例生效，默认 0（禁用）",
  "shutdown-on-sigint": "接收 SIGINT 信号时的关闭行为，可选 default（默认）、save、nosave、now、force，可组合（save 和 nosave 不可同时使用）",
  "shutdown-on-sigterm": "接收 SIGTERM 信号时的关闭行为，可选值同 shutdown-on-sigint，默认 default",
  "lua-time-limit": "EVAL 脚本、函数及部分模块命令的最大执行时间（毫秒），默认 5000，0 或负值表示无限制，超时后 Redis 仅允许部分命令执行",
  "busy-reply-threshold": "与 lua-time-limit 功能相同，为兼容旧配置的别名，默认 5000",
  "cluster-enabled": "是否启用 Redis Cluster 模式，默认 no，启用后实例才可加入集群",
  "cluster-config-file": "集群配置文件路径，由 Redis 自动创建和更新，不可手动编辑，默认 nodes-6379.conf",
  "cluster-node-timeout": "集群节点超时时间（毫秒），节点不可达超过此时间则标记为故障，默认 15000",
  "cluster-port": "集群总线监听端口，默认 0（命令端口 + 10000），手动配置后需在 cluster meet 命令中指定",
  "cluster-replica-validity-factor": "从节点故障转移的有效性因子，用于判断从节点数据是否过旧，默认 10，0 表示忽略数据年龄",
  "cluster-migration-barrier": "从节点迁移到无从节点主节点的迁移屏障，默认 1（原主节点至少保留 1 个从节点时才迁移），0 仅用于调试，大值禁用迁移",
  "cluster-allow-replica-migration": "是否允许从节点自动迁移，默认 yes，禁用则 cluster-migration-barrier 失效",
  "cluster-require-full-coverage": "集群是否需要所有哈希槽均被覆盖才接受查询，默认 yes，no 则部分槽可用时仍可查询对应键空间",
  "cluster-replica-no-failover": "从节点是否禁止自动故障转移，默认 no，yes 仅允许手动故障转移",
  "cluster-allow-reads-when-down": "集群故障时，节点是否仍为自身负责的槽提供读服务，默认 no，yes 适用于无需强一致性的场景（如缓存）",
  "cluster-allow-pubsubshard-when-down": "集群故障时，节点是否仍为自身负责的槽提供 Pub/Sub 服务，默认 yes",
  "cluster-link-sendbuf-limit": "集群总线连接的发送缓冲区内存限制（字节），默认 0（禁用），建议最小 1gb，用于防止缓冲区无限增长",
  "cluster-announce-hostname": "集群节点对外宣布的主机名，用于 SNI 或 DNS 路由，默认空字符串",
  "cluster-announce-human-nodename": "集群节点的人性化名称，用于调试和管理，默认空字符串",
  "cluster-preferred-endpoint-type": "集群对外提供的首选连接端点类型，可选 ip（默认）、hostname、unknown-endpoint",
  "cluster-compatibility-sample-ratio": "集群模式下命令兼容性检查的采样比例（0-100），默认 0（不采样），100 表示全量检查，高比例可能影响性能",
  "cluster-slot-stats-enabled": "是否启用哈希槽资源统计，默认 no，启用后可通过 CLUSTER SLOT-STATS 获取详细统计信息（如内存使用）",
  "cluster-slot-migration-write-pause-timeout": "槽迁移过程中，源节点暂停写入的超时时间（毫秒），默认 10000，防止写入长期阻塞",
  "cluster-slot-migration-handoff-max-lag-bytes": "槽迁移触发切换阶段的最大复制延迟（字节），默认 1mb，剩余延迟低于此值时源节点暂停写入",
  ...
  "slowlog-log-slower-than": "慢查询日志的阈值（微秒），超过此时间的命令会被记录，1000000 微秒 = 1 秒，负值禁用，0 记录所有命令，默认 10000",
  "slowlog-max-len": "慢查询日志的最大条目数，超出则删除最旧条目，默认 128",
  "latency-monitor-threshold": "延迟监控的阈值（毫秒），超过此时间的操作会被采样，0 禁用监控，默认 0",
  "latency-tracking": "是否启用扩展延迟监控（跟踪每个命令的延迟分布），默认 yes",
  "latency-tracking-info-percentiles": "扩展延迟监控通过 INFO latencystats 输出的百分位数，默认 50、99、99.9",
  "notify-keyspace-events": "键空间事件通知配置，由字符组合表示启用的事件类型（如 K、E、g、$ 等），空字符串禁用通知，默认\"\"",
  "hash-max-listpack-entries": "哈希对象使用紧凑编码的最大条目数，超出则转为哈希表，默认 512",
  "hash-max-listpack-value": "哈希对象使用紧凑编码的最大值大小（字节），超出则转为哈希表，默认 64",
  "list-max-listpack-size": "列表对象使用紧凑编码的节点大小限制，负值表示按字节（-1=4Kb、-2=8Kb 等），正值表示按元素数，默认 - 2",
  ...
  "hll-sparse-max-bytes": "HyperLogLog 稀疏表示的最大字节数（含 16 字节头部），超出则转为密集表示，默认 3000，最大 16000",
  ...
  "client-output-buffer-limit normal": "普通客户端（含 MONITOR）的输出缓冲区限制，格式为 “硬限制 软限制 软限制持续时间”，默认 0 0 0（无限制）",
  ...
  "client-query-buffer-limit": "客户端查询缓冲区的最大大小，默认 1gb，用于限制协议不同步导致的内存占用",
  "lookahead": "每个客户端流水线中预解码和预获取的命令数，默认 16",
  "maxmemory-clients": "所有客户端连接占用的最大内存限制，0 禁用客户端淘汰，支持绝对值（如 1g）或百分比（如 5%），默认 0",
  ...
  "aof-rewrite-incremental-fsync": "AOF 重写时是否每生成 4MB 数据执行一次 fsync，默认 yes，用于减少延迟峰值",
  "rdb-save-incremental-fsync": "RDB 保存时是否每生成 4MB 数据执行一次 fsync，默认 yes，用于减少延迟峰值",
  ...
  "active-defrag-max-scan-fields": "每次主字典扫描处理的集合 / 哈希 / 有序集合 / 列表字段数，默认 1000",
...
const enConfigTip = {
  "include": "Include one or more other configuration files, support wildcards, included files are loaded in alphabetical order, no error is reported when no files match the wildcard",
  "loadmodule": "Load modules at startup, multiple modules can be specified, the server will abort if module loading fails, and parameters can be passed to modules",
  ...
  "bind-source-addr": "Configure the local address bound to outgoing connections, affecting the connection routing method",
  "protected-mode": "Security protection mode. When enabled and the default user has no password, only local connections (127.0.0.1, ::1 or Unix domain sockets) are allowed, enabled by default",
  "enable-protected-configs": "Control the modifiable permissions of sensitive configuration directives, optional values: no (default, disable modification for all connections), yes (allow modification for all connections), local (allow modification only for local connections)",
  "enable-debug-command": "Control the enablement permissions of debug commands, optional values same as enable-protected-configs, default no",
  "enable-module-command": "Control the enablement permissions of module commands, optional values same as enable-protected-configs, default no",
  "port": "The TCP port to listen on, default 6379, setting to 0 means not listening on a TCP socket",
  "tcp-backlog": "The length of the TCP listen queue, the actual value is limited by the system /proc/sys/net/core/somaxconn, default 511",
  "unixsocket": "The path of the Unix domain socket, if not specified, Redis will not listen on a Unix domain socket",
  "unixsocketperm": "The permissions of the Unix domain socket, default 700",
  "timeout": "Client idle timeout (seconds), 0 means disabling timeout to close connections, default 0",
  "tcp-keepalive": "TCP keepalive time (seconds), a non-zero value enables the SO_KEEPALIVE option, used to detect dead connections and maintain network connections, default 300",
  "socket-mark-id": "Mark the listening socket with an ID to support advanced routing and filtering. On Linux, it corresponds to a connection mark; on FreeBSD, it corresponds to a socket cookie ID; on OpenBSD, it corresponds to a route table ID, default 0",
  "tls-port": "TLS listening port, TLS needs to be configured with this parameter to enable, TLS is disabled by default",
  "tls-cert-file": "Path to the server-side X.509 certificate file (PEM format), used to authenticate to clients, master nodes or cluster nodes",
  ...
  "tls-client-cert-file": "Path to the client-side (used for replication, cluster connections, etc.) X.509 certificate file (PEM format)",
  ...
  "tls-dh-params-file": "Path to the DH parameters file, used to support Diffie-Hellman key exchange for older versions of OpenSSL (<3.0), not recommended for new versions",
  "tls-ca-cert-file": "Path to the CA certificate file, used to authenticate TLS clients and nodes, Redis does not use the system CA configuration by default",
  "tls-ca-cert-dir": "Path to the CA certificate directory, same function as tls-ca-cert-file",
  "tls-auth-clients": "Control the requirement for TLS client certificate authentication, optional values: no (do not require or accept client certificates), optional (optional, valid if provided), client certificates are required by default",
  ...
  "tls-protocols": "Supported TLS versions, optional TLSv1, TLSv1.1, TLSv1.2, TLSv1.3 (OpenSSL ≥ 1.1.1), TLSv1.2 and TLSv1.3 are enabled by default",
  "tls-ciphers": "Cipher suites supported for TLSv1.2 and below, syntax refers to the ciphers(1ssl) manual, default DEFAULT:!MEDIUM",
  "tls-ciphersuites": "Cipher suites supported for TLSv1.3, syntax refers to the ciphers(1ssl) manual",
  "tls-prefer-server-ciphers": "Whether to prefer the server-side cipher suite preference, default no (follow client preference)",
  "tls-session-caching": "Whether to enable TLS session caching, default yes, used to speed up client reconnections",
  "tls-session-cache-size": "Maximum number of TLS sessions cached, 0 means unlimited, default 20480",
  "tls-session-cache-timeout": "Timeout of cached TLS sessions (seconds), default 300",
  "daemonize": "Whether to run in daemon mode, default no, a pid file will be generated in daemon mode",
  "supervised": "Specify the supervision system interaction method, optional no (default, no interaction), upstart, systemd, auto (automatic detection)",
  "pidfile": "Path to the pid file, no pid file is generated if not specified in non-daemon mode, default /var/run/redis.pid in daemon mode, /run/redis.pid is recommended for modern Linux systems",
  "loglevel": "Log verbosity level, optional debug, verbose, notice (default, recommended for production environments), warning, nothing",
  "logfile": "Path to the log file, an empty string means output to standard output, log output to standard output will be directed to /dev/null in daemon mode",
  ...
  "crash-log-enabled": "Whether to enable crash logging, enabled by default, disabling can generate cleaner core dumps",
  "crash-memcheck-enabled": "Whether to enable the fast memory check of crash logs, enabled by default, disabling can make Redis terminate faster",
  "databases": "Number of databases, 16 by default (numbered 0-15), can be switched using the SELECT command",
  "always-show-logo": "Whether to always display the ASCII art logo in startup logs, default no, only displayed by default in interactive sessions",
  "hide-user-data-from-log": "Whether to prohibit logging personally identifiable information (PII), disabled by default",
  "set-proc-title": "Whether to modify the process title to display runtime information, default yes",
  "proc-title-template": "Process title template, supporting variables {title}, {listen-addr}, {server-mode}, {port}, {tls-port}, {unixsocket}, {config-file}, default \"{title} {listen-addr} {server-mode}\"",
  "locale-collate": "Local environment used for string comparison operations, an empty string means obtained from environment variables, affecting Lua script performance",
  ...
  "stop-writes-on-bgsave-error": "Whether to stop accepting writes if background saving fails when RDB snapshots are enabled, default yes, ensuring normal data persistence",
  "rdbcompression": "Whether to compress string objects in RDB files using LZF, default yes, disabling can save CPU but increase file size",
  "rdbchecksum": "Whether to add a CRC64 checksum at the end of the RDB file, default yes, disabling can improve read and write performance but reduce file integrity check capability",
  "sanitize-dump-payload": "Whether to perform full verification when loading RDB or RESTORE data, optional no (default), yes, clients, used to reduce the risk of assertions or crashes during subsequent command execution",
  "dbfilename": "Name of the RDB file, default dump.rdb",
  "rdb-del-sync-files": "Whether to delete RDB files generated during replication when no persistence is enabled, default no, only effective when both AOF and RDB are disabled",
  "dir": "Working directory, RDB files and AOF files are stored in this directory, default ./",
  "replicaof": "Configure the current instance as a replica node, specifying the IP and port of the master node",
  "masterauth": "Password of the master node, required when the master node has a password enabled",
  "masteruser": "Username of the master node used for replication, used when Redis 6 and above support ACL, for executing commands required for replication such as PSYNC",
  "replica-serve-stale-data": "Whether to return stale data when the replica node is disconnected from the master node or during replication, default yes, setting to no will reject data access commands except for some commands",
  "replica-read-only": "Whether the replica node is read-only, default yes, read-only by default since Redis 2.6",
  "repl-diskless-sync": "Synchronization strategy used for full replication, yes means diskless synchronization (transfer RDB directly through the socket), no means disk-based synchronization, default yes",
  "repl-diskless-sync-delay": "Delay time (seconds) for waiting for more replica nodes to connect during diskless synchronization, default 5, setting to 0 starts transmission immediately",
  "repl-diskless-sync-max-replicas": "Start synchronization in advance when the specified number of replica nodes connect during the diskless synchronization delay period, default 0 means no limit",
  "repl-diskless-load": "The way the replica node loads RDB data from the master node, optional disabled (default, store to disk first then load), swapdb (load in parallel in memory, requiring sufficient memory), on-empty-db (diskless load only when the database is empty)",
  "repl-ping-replica-period": "Interval time (seconds) for the master node to send PING to the replica node, default 10",
  "repl-timeout": "Replication timeout time (seconds), covering scenarios such as SYNC bulk transfer, master-replica node communication, default 60, which needs to be greater than repl-ping-replica-period",
  "repl-disable-tcp-nodelay": "Whether to disable the TCP_NODELAY option of the replica node socket after full synchronization, default no (prioritizing low latency), yes can reduce bandwidth usage but increase data latency",
  "repl-backlog-size": "Size of the replication backlog buffer, used for partial synchronization after the replica node reconnects, default 1mb, only allocated when there are replica nodes connected",
  "repl-backlog-ttl": "Release time (seconds) of the replication backlog buffer after the last replica node disconnects, default 3600, 0 means never release",
  "replica-full-sync-buffer-limit": "Maximum size of replication data stream that the replica node can accumulate during full synchronization, 0 means inheriting the hard limit of the master node's client-output-buffer-limit replica, default 0",
  "replica-priority": "Replica node priority, used by Redis Sentinel to select a master node during failover, the smaller the value, the higher the priority, 0 means cannot be promoted to master node, default 100",
  "propagation-error-behavior": "Behavior for handling command execution errors in the replication stream or AOF file, optional ignore (default), panic, panic-on-replicas",
  "replica-ignore-disk-write-errors": "Whether the replica node ignores errors when writing to disk fails, default no (crash), yes only logs a warning and executes the command",
  "replica-announced": "Whether the replica node is included in Redis Sentinel's reports, default yes, setting to no will not be displayed by the sentinel replicas command, but can still be promoted to master node (need to set replica-priority 0 to disable promotion)",
  "min-replicas-to-write": "Minimum number of online replica nodes required for the master node to accept writes, default 0 (disabled), need to be used with min-replicas-max-lag",
  "min-replicas-max-lag": "Maximum latency time (seconds) of replica nodes when the master node accepts writes, default 10",
  "replica-announce-ip": "IP address reported by the replica node to the master node, used in NAT or port forwarding scenarios",
  "replica-announce-port": "Port reported by the replica node to the master node, used in NAT or port forwarding scenarios",
  "tracking-table-max-keys": "Maximum number of keys in the invalidation table for client-side cache key tracking, default 1000000, 0 means no limit, this configuration is invalid in broadcast mode",
  "acllog-max-len": "Maximum number of entries in the ACL log, used to record failed commands and authentication events related to ACL, default 128",
  "aclfile": "Path to the external ACL user configuration file, cannot be used simultaneously with user configuration in the current configuration file",
  "requirepass": "Password of the default user, a compatibility layer of the ACL system in Redis 6 and above, incompatible with the aclfile and ACL LOAD commands",
  "acl-pubsub-default": "Default Pub/Sub channel permissions for new users, optional allchannels (allow all channels), resetchannels (forbid all channels), resetchannels is default in Redis 7.0",
  "rename-command": "Rename or disable commands (deprecated), it is recommended to use ACL to control command permissions, format \"rename-command original-command new-command\", setting the new command to an empty string disables the command",
  "maxclients": "Maximum number of simultaneous connected clients, default 10000, actually limited by the system file descriptor limit, cluster bus connection resources need to be reserved in Redis Cluster mode",
  "maxmemory": "Maximum memory limit used by Redis, when the limit is reached, key eviction is performed according to maxmemory-policy, no limit by default",
  "maxmemory-policy": "Key eviction policy when memory reaches maxmemory, optional volatile-lru, allkeys-lru, volatile-lfu, allkeys-lfu, volatile-random, allkeys-random, volatile-ttl, noeviction (default)",
  "maxmemory-samples": "Sampling number of LRU/LFU/TTL eviction algorithms, default 5, the larger the value, the closer to the real result but the higher the CPU consumption, maximum 64",
  "maxmemory-eviction-tenacity": "Aggressiveness of key eviction processing, 0 for minimum latency, 10 for default, 100 for prioritizing eviction regardless of latency, default 10",
  "replica-ignore-maxmemory": "Whether the replica node ignores its own maxmemory configuration, default yes (eviction is handled by the master node), modify only when the replica node is writable or needs an independent memory limit",
  "active-expire-effort": "Intensity of active expired key cleaning, default 1, value range 1-10, the larger the value, the more thorough the cleaning but the higher the CPU consumption",
  "lazyfree-lazy-eviction": "Whether to use lazy deletion (non-blocking) during memory eviction, default no",
  "lazyfree-lazy-expire": "Whether to use lazy deletion when keys expire, default no",
  "lazyfree-lazy-server-del": "Whether to use lazy deletion when the server internally deletes keys (such as RENAME overwriting), default no",
  "replica-lazy-flush": "Whether the replica node lazily clears the database during full synchronization, default no",
  "lazyfree-lazy-user-del": "Whether the user uses lazy deletion when executing the DEL command, default no",
  "lazyfree-lazy-user-flush": "Whether to delete asynchronously by default when the user executes FLUSHDB/FLUSHALL/SCRIPT FLUSH/FUNCTION FLUSH without specifying the sync/async flag, default no",
  "io-threads": "Number of I/O threads, used to handle reading/writing of client sockets and protocol parsing, disabled by default (1 means only the main thread), it is recommended to enable on servers with 4 or more cores, leaving 1 idle core",
  "oom-score-adj": "Whether to control the process priority of the kernel OOM killer, optional no (default), yes (equivalent to relative), absolute, relative, used to prioritize evicting background child processes and replica nodes",
  "oom-score-adj-values": "OOM priority adjustment values, in order of master node, replica node, background child process, value range -2000 to 2000, default 0 200 800",
  "disable-thp": "Whether to disable Transparent Huge Pages (THP) for the Redis process, default yes, avoiding latency issues caused by fork and Copy-on-Write (CoW)",
  "appendonly": "Whether to enable AOF persistence, default no, AOF and RDB can be enabled simultaneously, AOF is loaded first at startup",
  "appendfilename": "Base name of the AOF file, Redis 7 and above use a multi-file format (base file, incremental file, manifest file), default \"appendonly.aof\"",
  "appenddirname": "Storage directory of AOF files, default \"appendonlydir\"",
  "appendfsync": "fsync policy of AOF files, optional always (sync after every write, safest), everysec (sync every second, default), no (controlled by the system, fastest)",
  "no-appendfsync-on-rewrite": "Whether to suspend AOF's fsync when BGSAVE or BGREWRITEAOF is executed, default no (prioritizing data security), yes can alleviate I/O blocking but reduce durability",
  "auto-aof-rewrite-percentage": "Trigger percentage of automatic AOF rewrite, based on the file size after the last rewrite, default 100 (grows by 100%), 0 disables automatic rewrite",
  "auto-aof-rewrite-min-size": "Minimum file size for automatic AOF rewrite, default 64mb",
  "aof-load-truncated": "Whether to continue loading when a truncated AOF file is found at startup, default yes (load available parts and log a prompt), no will directly report an error",
  "aof-load-corrupt-tail-max-size": "Maximum number of corrupted bytes at the end of the AOF file that Redis can automatically recover at startup, default 0 (disable automatic recovery), manual processing is required if exceeded",
  "aof-use-rdb-preamble": "Whether the AOF base file uses the RDB format, default yes, RDB format is faster and more efficient",
  "aof-timestamp-enabled": "Whether to record timestamp annotations in AOF to support point-in-time recovery, default no, enabling may be incompatible with some AOF parsers",
  "shutdown-timeout": "Maximum time (seconds) to wait for replica nodes to synchronize when shutting down, only effective for instances with replica nodes, default 0 (disabled)",
  "shutdown-on-sigint": "Shutdown behavior when receiving the SIGINT signal, optional default, save, nosave, now, force, can be combined (save and nosave cannot be used simultaneously)",
  "shutdown-on-sigterm": "Shutdown behavior when receiving the SIGTERM signal, optional values same as shutdown-on-sigint, default default",
  "lua-time-limit": "Maximum execution time (milliseconds) of EVAL scripts, functions and some module commands, default 5000, 0 or negative value means no limit, Redis only allows some commands to be executed after timeout",
  "busy-reply-threshold": "Same function as lua-time-limit, an alias for compatible with old configurations, default 5000",
  "cluster-enabled": "Whether to enable Redis Cluster mode, default no, instances can join the cluster only after enabling",
  "cluster-config-file": "Path to the cluster configuration file, automatically created and updated by Redis, cannot be edited manually, default nodes-6379.conf",
  "cluster-node-timeout": "Cluster node timeout time (milliseconds), a node is marked as faulty if unreachable for more than this time, default 15000",
  "cluster-port": "Cluster bus listening port, default 0 (command port + 10000), need to specify the cluster bus port in the cluster meet command after manual configuration",
  "cluster-replica-validity-factor": "Validity factor for replica node failover, used to determine if the replica node data is too old, default 10, 0 means ignoring data age",
  "cluster-migration-barrier": "Migration barrier for replica nodes to migrate to master nodes without replica nodes, default 1 (migrate only if the original master node retains at least 1 replica node), 0 is only for debugging, large values disable migration",
  "cluster-allow-replica-migration": "Whether to allow automatic migration of replica nodes, default yes, disabling makes cluster-migration-barrier invalid",
  "cluster-require-full-coverage": "Whether the cluster needs all hash slots to be covered to accept queries, default yes, no allows queries for the corresponding key space when some slots are available",
  "cluster-replica-no-failover": "Whether to prohibit replica nodes from automatic failover, default no, yes only allows manual failover",
  "cluster-allow-reads-when-down": "Whether nodes still provide read services for the slots they are responsible for when the cluster fails, default no, yes is suitable for scenarios that do not require strong consistency (such as caches)",
  "cluster-allow-pubsubshard-when-down": "Whether nodes still provide Pub/Sub services for the slots they are responsible for when the cluster fails, default yes",
  "cluster-link-sendbuf-limit": "Memory limit (bytes) for the send buffer of cluster bus connections, default 0 (disabled), minimum 1gb is recommended to prevent unlimited buffer growth",
  "cluster-announce-hostname": "Hostname announced by cluster nodes to the outside, used for SNI or DNS routing, default empty string",
  "cluster-announce-human-nodename": "Human-readable name of cluster nodes, used for debugging and management, default empty string",
  "cluster-preferred-endpoint-type": "Preferred connection endpoint type provided by the cluster to the outside, optional ip (default), hostname, unknown-endpoint",
  "cluster-compatibility-sample-ratio": "Sampling ratio (0-100) for command compatibility checks in cluster mode, default 0 (no sampling), 100 means full check, high ratio may affect performance",
  "cluster-slot-stats-enabled": "Whether to enable hash slot resource statistics, default no, enabling allows obtaining detailed statistics (such as memory usage) through CLUSTER SLOT-STATS",
  "cluster-slot-migration-write-pause-timeout": "Timeout time (milliseconds) for the source node to pause writes during the slot migration handoff phase, default 10000, preventing long-term write blocking",
  "cluster-slot-migration-handoff-max-lag-bytes": "Maximum replication lag (bytes) for triggering the slot handoff phase during slot migration, default 1mb, the source node pauses writes when the remaining lag is below this value",
  "cluster-announce-ip": "IP address announced by cluster nodes to the outside, used in NAT or container environments",
  "cluster-announce-tls-port": "TLS port announced by cluster nodes to the outside, effective when tls-cluster is enabled",
  "cluster-announce-bus-port": "Bus port announced by cluster nodes to the outside",
  "slowlog-log-slower-than": "Threshold (microseconds) for slow query logs, commands exceeding this time will be recorded, 1000000 microseconds = 1 second, negative value disables, 0 records all commands, default 10000",
  "slowlog-max-len": "Maximum number of entries in the slow query log, the oldest entry is deleted when exceeded, default 128",
  "latency-monitor-threshold": "Threshold (milliseconds) for latency monitoring, operations exceeding this time will be sampled, 0 disables monitoring, default 0",
  "latency-tracking": "Whether to enable extended latency monitoring (tracking the latency distribution of each command), default yes",
  "latency-tracking-info-percentiles": "Percentiles output by extended latency monitoring through INFO latencystats, default 50, 99, 99.9",
  "notify-keyspace-events": "Keyspace event notification configuration, composed of characters to indicate enabled event types (such as K, E, g, $, etc.), empty string disables notifications, default \"\"",
  "hash-max-listpack-entries": "Maximum number of entries for hash objects to use compact encoding, converted to hash table when exceeded, default 512",
  "hash-max-listpack-value": "Maximum value size (bytes) for hash objects to use compact encoding, converted to hash table when exceeded, default 64",
  "list-max-listpack-size": "Node size limit for list objects to use compact encoding, negative values represent by bytes (-1=4Kb, -2=8Kb, etc.), positive values represent by the number of elements, default -2",
  "list-compress-depth": "Compression depth of list objects, 0 disables compression, positive values indicate the number of nodes not compressed at both ends, default 0",
  "set-max-intset-entries": "Maximum number of entries for set objects to use integer set encoding, converted to hash table when exceeded, default 512",
  "set-max-listpack-entries": "Maximum number of entries for non-integer sets to use compact encoding, converted to hash table when exceeded, default 128",
  "set-max-listpack-value": "Maximum value size (bytes) for non-integer sets to use compact encoding, converted to hash table when exceeded, default 64",
  "zset-max-listpack-entries": "Maximum number of entries for sorted sets to use compact encoding, converted to skip list when exceeded, default 128",
  "zset-max-listpack-value": "Maximum value size (bytes) for sorted sets to use compact encoding, converted to skip list when exceeded, default 64",
  "hll-sparse-max-bytes": "Maximum bytes for the sparse representation of HyperLogLog (including 16-byte header), converted to dense representation when exceeded, default 3000, maximum 16000",
  "stream-node-max-bytes": "Maximum bytes of stream object nodes, 0 means no limit, default 4096",
  "stream-node-max-entries": "Maximum number of entries of stream object nodes, 0 means no limit, default 100",
  "activerehashing": "Whether to enable active rehashing, default yes, used to release free memory of hash tables, can be disabled for latency-sensitive scenarios",
  "client-output-buffer-limit normal": "Output buffer limit for normal clients (including MONITOR), format \"hard limit soft limit soft limit duration\", default 0 0 0 (no limit)",
  "client-output-buffer-limit replica": "Output buffer limit for replica node clients, default 256mb 64mb 60",
  "client-output-buffer-limit pubsub": "Output buffer limit for Pub/Sub clients, default 32mb 8mb 60",
  "client-query-buffer-limit": "Maximum size of the client query buffer, default 1gb, used to limit memory usage caused by protocol desynchronization",
  "lookahead": "Number of commands to decode and prefetch in each client pipeline, default 16",
  "maxmemory-clients": "Maximum memory limit occupied by all client connections, 0 disables client eviction, supports absolute values (such as 1g) or percentages (such as 5%), default 0",
  ...
  "dynamic-hz": "Whether to enable dynamic HZ adjustment, default yes, automatically increase HZ when there are many client connections and decrease when idle",
  "aof-rewrite-incremental-fsync": "Whether to execute fsync every 4MB of data generated during AOF rewrite, default yes, used to reduce latency spikes",
  "rdb-save-incremental-fsync": "Whether to execute fsync every 4MB of data generated during RDB saving, default yes, used to reduce latency spikes",
  "lfu-log-factor": "Counter logarithm factor of the LFU eviction algorithm, default 10, affecting the growth speed of the counter",
  "lfu-decay-time": "Decay time (minutes) of the LFU counter, default 1, 0 means never decay",
  "max-new-connections-per-cycle": "Maximum number of new connections that can be accepted per event loop cycle (non-TLS), default 10",
  "max-new-tls-connections-per-cycle": "Maximum number of new TLS connections that can be accepted per event loop cycle, default 1",
  "activedefrag": "Whether to enable active memory defragmentation, default no, only supports the Jemalloc allocator built into Redis",
  "active-defrag-ignore-bytes": "Minimum fragmented waste bytes for active defragmentation, default 100mb",
  "active-defrag-threshold-lower": "Minimum fragmentation rate threshold (percentage) for active defragmentation, default 10",
  "active-defrag-threshold-upper": "Maximum fragmentation rate threshold (percentage) for active defragmentation, default 100",
  "active-defrag-cycle-min": "CPU usage percentage (percentage) when the fragmentation rate reaches the lower threshold, default 1",
  "active-defrag-cycle-max": "CPU usage percentage (percentage) when the fragmentation rate reaches the upper threshold, default 25",
  "active-defrag-max-scan-fields": "Number of set/hash/sorted set/list fields processed per main dictionary scan, default 1000",
  ...
  "aof-rewrite-cpulist": "CPU affinity of the AOF rewrite child process, format same as the taskset command",
  "bgsave-cpulist": "CPU affinity of the BGSAVE child process, format same as the taskset command",
  "ignore-warnings": "List of Redis startup warnings to suppress, separated by spaces, such as ARM64-COW-BUG"
}
const enClientTip = {
